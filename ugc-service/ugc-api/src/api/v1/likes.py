...
@router.delete("/movies/{movie_id}/likes/{like_id}")
async def delete(
    like_id: UUID,
    movie_id: UUID,
    user_credentials: CustomHTTPAuthorizationCredentials = Depends(TokenCheck()),
...
async def get(like_id: UUID, service: BaseLikesService = Depends(get_likes_service)):
    return handle_result(
...
async def get_list(
    movie_id: Optional[UUID], service: BaseLikesService = Depends(get_likes_service)
):
    return handle_result(
